...
$Ide = $_GET['Ide'];
$usrNum = $_GET['usrNum'];
$query = "SELECT * FROM 通知 WHERE 學號 = '$usrNum' ORDER BY 事件編號 DESC";
...
<body>
    ...
    <div class="container">
        <h1 class="text-center" style="color:#5151A2; font-weight:bold;background-color:#ECF5FF;">個人通知</h1>
        <p></p>
        <p></p>
            ...
            <tbody>
                ...
                ?>
                        <tr>
                            ...
                            <td style="vertical-align:middle;"><?php echo $row['通知內容']; ?></td>
                            <td style="vertical-align:middle;"><?php echo ($row['已讀'] == '是') ? '已讀' : '未讀'; ?></td>
                            <td style="vertical-align:middle;">
                                <?php
                                if ($row['已讀'] == '是') { ?>
                                    <button type="button" class="btn btn-secondary">已讀</button>
                                <?php
                                } else { ?>
                                    <a href="noticesql_edit.php?Acc=<?= $Acc ?>&Ide=<?= $Ide ?>&ID=<?= $phpID ?>&usrNum=<?= $usrNum ?>" class="btn-link"><button type="button" class="btn btn-primary">已讀</button></a>
                                <?php
                                } ?>
