...
$name = $result['姓名'];
$equipCheck = "SELECT * FROM `設備` INNER JOIN 
    (SELECT `包含`.`宿舍大樓名稱`, `包含`.`房間號碼`, `包含`.`設備編號` FROM `包含` INNER JOIN `學生` 
    ON `學生`.`宿舍大樓名稱` = `包含`.`宿舍大樓名稱` AND `學生`.`房間號碼` = `包含`.`房間號碼` 
    WHERE `學生`.`姓名` = '$name') AS `子查詢` USING (`設備編號`); ";
$equipAll = mysqli_query($link, $equipCheck);
//$equip=mysqli_fetch_assoc($equipAll);
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-6 text-center" style="background-color:#ECF5FF; border-bottom:#5151A2 solid 2px;">
                <p></p>
                <h1 class="text-center" style="color:#5151A2; font-weight:bold;">設備報修</h1>
            </div>
            <div class="col-3"></div>
        </div>
        <div class="row">
            ...
            <div class="col-6 text-center" style="background-color:#ECF5FF;">
                <p></p>
                <form method="post" action="applysql_repair.php?Acc=<?= $Acc ?>&Ide=<?= $Ide ?>" style="color:#5151A2; font-weight:bold; font-size:20px;">
                    <div class="row mb-3">
                    ...
                    <div class="row mb-3">
                        <label for="inputPassword" class="col-sm-3 col-form-label">學號</label>
                        <br>
                        <div class="col-sm-9" style="margin:auto;">
                            <input type="text" class="form-control" name="usrNum" value="<?php echo $result['學號'] ?>" readonly="readonly" required>
                        </div>
                    </div>
                    <div class="row mb-3">
                    ...
                    <div class="row mb-3">
                        <label class="col-sm-3 col-form-label">報修設備</label>
                        <div class="col-sm-9">
                            <select class="form-control" name="usrEqu" aria-label="Default select" required>
                                <option value="" selected>請選擇設備</option>
                                <?php
                                if (mysqli_num_rows($equipAll) > 0) {
                                    foreach ($equipAll as $row) {
                                ?>
                                        <option value="<?php echo $row['設備編號'] ?>"><?php echo $row['宿舍大樓名稱'] ?> - <?php echo $row['房間號碼'] ?> - <?php echo $row['設備名稱'] ?></option>
                                <?php
                                    }
                                }
                                ?>
                    ...
                    <div class="text-end">
                        <a class="btn btn-outline-primary" href="repair_progress.php?Acc=<?= $Acc ?>&Ide=<?= $Ide ?>" role="button">報修進度查詢</a>
