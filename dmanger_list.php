...
<html>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark fixed-top" style = "background-color:#0066CCAA;">
        ...
        <div class = "container">
            <p></p>
            <h1 class="text-center" style = "color:#5151A2; font-weight:bold;background-color:#ECF5FF;">舍監名單</h1>
            <a href="dmanger_register.php?Acc=<?=$usrAcc?>&Ide=<?=$value?>" class="btn-link"><button type="button" class="btn col-sm-1 offset-11" style ="background-color:#5151A2; color:white; font-weight:bold;">新增舍監</button></a>

            <p></p>
        <table class="table table-striped text-center table-sm table-responsive-md">
            <thead>
                <tr>
                    <th scope="col">舍監姓名</th>
			...
			?>
							<tr>
                                <?php $phpID=$row['舍監帳號'];?>
								<td style = "vertical-align:middle;"><?php echo $row['舍監姓名']; ?></td> 
								<td style = "vertical-align:middle;"><?php echo $row['Email']; ?></td>
								<td style = "vertical-align:middle;"><?php echo $row['連絡電話']; ?></td>
								<td style = "vertical-align:middle;"><?php echo $row['舍監帳號']; ?></td> 
								<td style = "vertical-align:middle;"><?php echo $row['密碼']; ?></td>
                                <td style = "vertical-align:middle;">
                                    <a href="dmanger_edit.php?Acc=<?=$usrAcc?>&ID=<?=$phpID?>&Ide=<?=$value?>" class="btn-link"><button type="button" class="btn btn-primary">編輯</button></a>
                                    <a href="dmangersql_delete.php?Acc=<?=$usrAcc?>&ID=<?=$phpID?>&Ide=<?=$value?>" class="btn-link"><button type="button" class="btn btn-danger">刪除</button></a>
                                </td>
							</tr>
			<?php
                    }
